...
context("Prediction Market Contract V3 Controller", async () => {
  ...
  context("Contract Deployment", async () => {
    ...
    it(
      "should deploy Prediction Market Controller Contract",
      mochaAsync(async () => {
        ...
        predictionMarketFactoryContract =
          app.getPredictionMarketV3FactoryContract({});
        // Deploy
        ...
        const predictionMarketFactoryContractAddress =
          predictionMarketFactoryContract.getAddress();
        ...
        const predictionMarketControllerAddress =
          await predictionMarketFactoryContract.getPMControllerAddressById({
            id: 0,
          });

        expect(predictionMarketControllerAddress).to.not.equal(null);

        predictionMarketControllerContract =
          app.getPredictionMarketV3ControllerContract({
            contractAddress: predictionMarketControllerAddress,
          });
  ...
  context("Land Management", async () => {
    context("Land Creation", async () => {
      it(
        ...
        mochaAsync(async () => {
          const currentLandIndex =
            await predictionMarketControllerContract.getLandTokensLength();
          ...
          const land = await predictionMarketControllerContract.getLandById({
            id: currentLandIndex,
          });
          const newLandIndex =
            await predictionMarketControllerContract.getLandTokensLength();
    ...
    context("Land Admins", async () => {
      ...
      before(
        mochaAsync(async () => {
          ...
          user1PredictionMarketControllerContract =
            user1App.getPredictionMarketV3ControllerContract({
      ...
      it(
        "should add an admin to a Land",
        mochaAsync(async () => {
          const currentIsAdmin =
            await predictionMarketControllerContract.isLandAdmin({
              token,
              user: user2,
            });
      ...
      it(
        "should remove an admin from a Land",
        mochaAsync(async () => {
          const currentIsAdmin =
            await predictionMarketControllerContract.isLandAdmin({
              token,
      ...
      it(
        "should not be able to add an admin to a Land if not an admin making the call",
        mochaAsync(async () => {
          const currentIsAdmin =
            await predictionMarketControllerContract.isLandAdmin({
              token,
      ...
      it(
        "should be able to add/remove an admin to a Land after being made admin",
        mochaAsync(async () => {
          await predictionMarketControllerContract.addAdminToLand({
            token,
            user: user1,
      ...
      it(
        "should be land admin if is a controller admin",
        mochaAsync(async () => {
          const currentIsAdmin =
            await predictionMarketControllerContract.isLandAdmin({
              token,
    ...
    context("Land Disabling + Enabling + Offset", async () => {
      ...
      before(
        mochaAsync(async () => {
          land = await predictionMarketControllerContract.getLandById({
            id: landId,
          });
          landTokenContract = app.getFantasyERC20Contract({
      ...
      it(
        "should not be able to disable a Land if not an admin making the call",
        mochaAsync(async () => {
          try {
          ...
          const refreshedLand =
            await predictionMarketControllerContract.getLandById({
              id: landId,
            });
      ...
      it(
        "should not be able to disable a Land if already disabled",
        mochaAsync(async () => {
          try {
            await predictionMarketControllerContract.disableLand({
              token,
  ...
  context("Land Markets", async () => {
    ...
    before(
      mochaAsync(async () => {
        ...
        land2 = await predictionMarketControllerContract.getLandById({
          id: landId + 1,
        });
        ...
        land2TokenContract = app.getFantasyERC20Contract({
          contractAddress: land2.token,
        });
        user1LandTokenContract = user1App.getFantasyERC20Contract({
          contractAddress: land.token,
        });
        user1Land2TokenContract = user1App.getFantasyERC20Contract({
          contractAddress: land2.token,
        });
    ...
    context("Market Creation", async () => {
      it(
        "should not be able to create a Market if not an admin making the call and everyoneCanCreateMarkets is not active",
        mochaAsync(async () => {
          const currentMarketIds = await predictionMarketContract.getMarkets();
          const currentLandTokenBalance = await landTokenContract.balanceOf({
            address: user1,
          });
          ...
          try {
            await user1PredictionMarketContract.mintAndCreateMarket({
              ...
              realitioTimeout: 300,
              PM3ManagerAddress:
                predictionMarketControllerContract.getAddress(),
            });
          } catch (e) {
            // not logging error, as tx is expected to fail
          }
      ...
      it(
        "should create a Market if admin",
        mochaAsync(async () => {
          const currentMarketIds = await predictionMarketContract.getMarkets();
          const currentLandTokenBalance = await landTokenContract.balanceOf({
            address: accountAddress,
          ...
          const newLandTokenBalance = await landTokenContract.balanceOf({
            address: accountAddress,
          });
          const newMarketIds = await predictionMarketContract.getMarkets();
      ...
      it(
        "should be able to create a Market if not an admin making the call and everyoneCanCreateMarkets is active",
        mochaAsync(async () => {
          const currentMarketIds = await predictionMarketContract.getMarkets();
          const currentLandTokenBalance = await land2TokenContract.balanceOf({
            address: user1,
          });
          expect(currentLandTokenBalance).to.equal(TOKEN_AMOUNT_TO_CLAIM);
          expect(
          ...
          const newLandTokenBalance = await land2TokenContract.balanceOf({
            address: accountAddress,
          });
          const newMarketIds = await predictionMarketContract.getMarkets();
          expect(newMarketIds.length).to.equal(currentMarketIds.length + 1);
    ...
    context("Market Interaction", async () => {
      it(
        "should be able to buy shares when land enabled",
        mochaAsync(async () => {
          ...
          const amountTransferred = Number(
            (currentLandTokenBalance - newLandTokenBalance).toFixed(5)
          );
      ...
      it(
        ...
        mochaAsync(async () => {
          ...
          const amountTransferred = Number(
            (newLandTokenBalance - currentLandTokenBalance).toFixed(5)
          );
      ...
      it(
        "should not be able to buy shares when land disabled",
        mochaAsync(async () => {
          const minOutcomeSharesToBuy = 0.015;
          const currentLandTokenBalance = await landTokenContract.balanceOf({
            address: accountAddress,
          ...
          const amountTransferred =
            currentLandTokenBalance - newLandTokenBalance;
      ...
      it(
        "should be able to buy shares when land enabled again",
        mochaAsync(async () => {
          const minOutcomeSharesToBuy = 0.015;
          const currentLandTokenBalance = await landTokenContract.balanceOf({
            address: accountAddress,
    ...
    context("Market Resolution", async () => {
      ...
      it(
        "should not be able to resolve a Market if not an admin making the call",
        mochaAsync(async () => {
          try {
          ...
          const market = await predictionMarketContract.getMarketData({
            marketId,
          });
      ...
      it(
        "should be able to resolve a Market",
        mochaAsync(async () => {
          try {
            const res =
              await predictionMarketContract.adminResolveMarketOutcome({
                marketId,
                outcomeId,
              });
            expect(res.status).to.equal(true);
          ...
          await landTokenContract.approve({
