...
context("Prediction Market Contract V3", async () => {
  ...
  context("Contract Deployment", async () => {
    ...
    it(
      "should deploy Prediction Market Contract",
      mochaAsync(async () => {
        accountAddress = await app.getAddress();

        // Create Contract
        predictionMarketContract = app.getPredictionMarketV3Contract({});
        predictionMarketManagerContract =
          app.getPredictionMarketV3ManagerContract({});
        ...
        // Deploy
        await requiredBalanceERC20Contract.deploy({
          params: ["Polkamarkets", "POLK"],
        });
        const requiredBalanceERC20ContractAddress =
          requiredBalanceERC20Contract.getAddress();
        ...
        await predictionMarketContract.deploy({
          params: [WETH9ContractAddress],
        });
        const predictionMarketContractAddress =
          predictionMarketContract.getAddress();
        ...
        const predictionMarketManagerContractAddress =
          predictionMarketManagerContract.getAddress();
        ...
        const land = await predictionMarketManagerContract.getLandById({
          id: 0,
        });
        // switching realitioERC20 contract addresses
        realitioERC20Contract = app.getRealitioERC20Contract({
          contractAddress: land.realitio,
        });
        realitioContractAddress = realitioERC20Contract.getAddress();

        tokenERC20Contract = app.getFantasyERC20Contract({
          contractAddress: land.token,
        });
  ...
  context("Market Data", async () => {
    ...
    it(
      "should get Market details",
      mochaAsync(async () => {
        const res = await predictionMarketContract.getMarketDetails({
          marketId,
        });
        expect(res).to.eql({
          name: "Will BTC price close above 100k$ on May 1st 2024",
    ...
    it(
      "should get Market Outcomes data",
      mochaAsync(async () => {
        const outcome1Data = await predictionMarketContract.getOutcomeData({
          marketId,
        ...
        const outcome2Data = await predictionMarketContract.getOutcomeData({
          marketId,
  ...
  context(
    "Market Interaction - Balanced Market (Same Outcome Odds)",
    async () => {
      let marketId = 0;
      it(
        "should add liquidity without changing shares balance",
        mochaAsync(async () => {
          const myShares = await predictionMarketContract.getMyMarketShares({
            marketId,
          });
          const marketData = await predictionMarketContract.getMarketData({
            marketId,
          });
          const outcomePrices = await predictionMarketContract.getMarketPrices({
            marketId,
          });
          ...
          try {
            const res = await predictionMarketContract.addLiquidity({
              marketId,
              value,
            });
            expect(res.status).to.equal(true);
          ...
          const myNewShares = await predictionMarketContract.getMyMarketShares({
            marketId,
          });
          const newMarketData = await predictionMarketContract.getMarketData({
            marketId,
          });
          const newOutcomePrices =
            await predictionMarketContract.getMarketPrices({ marketId });

          expect(newMarketData.liquidity).to.above(marketData.liquidity);
          expect(newMarketData.liquidity).to.equal(
            marketData.liquidity + value
          );

          // Outcome prices shoud remain the same after providing liquidity
          expect(newOutcomePrices.outcomes[0]).to.equal(
            outcomePrices.outcomes[0]
          );
          expect(newOutcomePrices.outcomes[1]).to.equal(
            outcomePrices.outcomes[1]
          );

          // Price balances are 0.5-0.5, liquidity will be added solely through liquidity shares
          expect(myNewShares.liquidityShares).to.above(
            myShares.liquidityShares
          );
          expect(myNewShares.liquidityShares).to.equal(
            myShares.liquidityShares + value
          );
          // shares balance remains the same
          expect(myNewShares.outcomeShares[0]).to.equal(
            myShares.outcomeShares[0]
          );
          expect(myNewShares.outcomeShares[1]).to.equal(
            myShares.outcomeShares[1]
          );
        })
      );

      it(
        "should remove liquidity without changing shares balance",
        mochaAsync(async () => {
          const myShares = await predictionMarketContract.getMyMarketShares({
            marketId,
          });
          ...
          const contractBalance = await tokenERC20Contract.balanceOf({
            address: predictionMarketContract.getAddress(),
          });
          ...
          try {
            const res = await predictionMarketContract.removeLiquidity({
              marketId,
              shares: value,
            });
            expect(res.status).to.equal(true);
          ...
          const newOutcomePrices =
            await predictionMarketContract.getMarketPrices({ marketId });
          const newContractBalance = await tokenERC20Contract.balanceOf({
            address: predictionMarketContract.getAddress(),
          });
          ...
          // Price balances are 0.5-0.5, liquidity will be added solely through liquidity shares
          expect(myNewShares.liquidityShares).to.below(
            myShares.liquidityShares
          );
          expect(myNewShares.liquidityShares).to.equal(
            myShares.liquidityShares - value
          ...
          const amountTransferred = Number(
            (contractBalance - newContractBalance).toFixed(5)
          );
          expect(amountTransferred).to.equal(value);
        })
      );
  ...
  context(
    "Market Interaction - Unbalanced Market (Different Outcome Odds)",
    async () => {
      let marketId = 0;
      it(
        "should display my shares",
        mochaAsync(async () => {
          const res = await predictionMarketContract.getMyMarketShares({
            marketId,
          });
      ...
      it(
        ...
        mochaAsync(async () => {
          ...
          const outcomeShares = await predictionMarketContract.getMarketShares({
            marketId,
          });
          const contractBalance = await tokenERC20Contract.balanceOf({
            address: predictionMarketContract.getAddress(),
          ...
          try {
            const res = await predictionMarketContract.buy({
              marketId,
              outcomeId,
              value,
          ...
          const newOutcomeShares =
            await predictionMarketContract.getMarketShares({ marketId });
          const newContractBalance = await tokenERC20Contract.balanceOf({
            address: predictionMarketContract.getAddress(),
          });

          // outcome price should increase
          expect(newOutcomePrices.outcomes[0]).to.above(
            outcomePrices.outcomes[0]
          );
          expect(newOutcomePrices.outcomes[0]).to.equal(0.8);
          // opposite outcome price should decrease
          expect(newOutcomePrices.outcomes[1]).to.below(
            outcomePrices.outcomes[1]
          );
          ...
          // 0.5 + 0.5 = 1
          expect(
            newOutcomePrices.outcomes[0] + newOutcomePrices.outcomes[1]
          ).to.equal(1);
          ...
          expect(newOutcomeShares.outcomes[0]).to.below(
            outcomeShares.outcomes[0]
          );
          expect(newOutcomeShares.outcomes[0]).to.equal(0.005);
          // opposite outcome shares should increase
          expect(newOutcomeShares.outcomes[1]).to.above(
          ...
          // 0.005 * 0.02 = 0.01^2
          expect(
            outcomeShares.outcomes[0] * outcomeShares.outcomes[1]
          ).to.equal(newMarketData.liquidity ** 2);
          expect(
            newOutcomeShares.outcomes[0] * newOutcomeShares.outcomes[1]
          ).to.equal(newMarketData.liquidity ** 2);
          ...
          const amountReceived = Number(
            (newContractBalance - contractBalance).toFixed(5)
          );
      ...
      it(
        ...
        mochaAsync(async () => {
          ...
          expect(newOutcomeShares.outcomes[0]).to.above(
            outcomeShares.outcomes[0]
          );
          ...
          expect(myNewShares.outcomeShares[0]).to.above(
            myShares.outcomeShares[0]
          );
      ...
      it(
        ...
        mochaAsync(async () => {
          ...
          try {
            const res = await predictionMarketContract.removeLiquidity({
              marketId,
              shares: liquiditySharesToRemove,
            });
            expect(res.status).to.equal(true);
          } catch (e) {
            console.log(e);
          ...
          expect(newOutcomeShares.outcomes[0]).to.equal(0.005);
          expect(newOutcomeShares.outcomes[1]).to.below(
            outcomeShares.outcomes[1]
          );
          expect(newOutcomeShares.outcomes[1]).to.equal(0.02);
          expect(
          ...
          expect(myNewShares.outcomeShares[1]).to.above(
            myShares.outcomeShares[1]
          );
      ...
      it(
        ...
        mochaAsync(async () => {
          ...
          try {
            const res = await predictionMarketContract.sell({
              marketId,
              outcomeId,
              value,
          ...
          expect(newOutcomePrices.outcomes[0]).to.below(
            outcomePrices.outcomes[0]
          );
          expect(newOutcomePrices.outcomes[0]).to.equal(0.5);
          // opposite outcome price should increase
          expect(newOutcomePrices.outcomes[1]).to.above(
            outcomePrices.outcomes[1]
          );
  ...
  context("Multiple Outcomes", async () => {
    ...
    context("Market Creation", async () => {
      it(
        "should create a Market with 3 outcomes",
        mochaAsync(async () => {
          try {
            const res = await predictionMarketContract.mintAndCreateMarket({
              value,
          ...
          const res = await predictionMarketContract.getMarketData({
            marketId,
          });
      ...
      it(
        "should create a Market with 32 outcomes",
        mochaAsync(async () => {
      ...
      it(
        "should not create a Market with more than 32 outcomes",
        mochaAsync(async () => {
    ...
    context("Market Interaction", async () => {
      ...
      it(
        ...
        mochaAsync(async () => {
          ...
          expect(newOutcomePrices.outcomes[2]).to.below(
            outcomePrices.outcomes[2]
          );
          expect(newOutcomePrices.outcomes[2]).to.equal(0.1);
          // Prices sum = 1
          // 0.1 + 0.1 + 0.8 = 1
          expect(
            newOutcomePrices.outcomes[0] +
              newOutcomePrices.outcomes[1] +
              newOutcomePrices.outcomes[2]
          ).to.equal(1);
          ...
          expect(newOutcomeShares.outcomes[1]).to.equal(0.02);
          expect(newOutcomeShares.outcomes[2]).to.above(
            outcomeShares.outcomes[2]
          );
          ...
          expect(
            outcomeShares.outcomes[0] *
              outcomeShares.outcomes[1] *
              outcomeShares.outcomes[2]
          ).to.equal(newMarketData.liquidity ** 3);
          expect(
            newOutcomeShares.outcomes[0] *
              newOutcomeShares.outcomes[1] *
              newOutcomeShares.outcomes[2]
          ).to.equal(newMarketData.liquidity ** 3);
      ...
      it(
        ...
        mochaAsync(async () => {
          ...
          expect(newOutcomePrices.outcomes[2]).to.equal(
            outcomePrices.outcomes[2]
          );
          ...
          expect(
            newOutcomeShares.outcomes[0] *
              newOutcomeShares.outcomes[1] *
              newOutcomeShares.outcomes[2]
          ).to.be.closeTo(newMarketData.liquidity ** 3, 0.00000001);
          ...
          expect(myNewShares.outcomeShares[1]).to.equal(0);
          expect(myNewShares.outcomeShares[2]).to.equal(
            myShares.outcomeShares[2]
          );
      ...
      it(
        ...
        mochaAsync(async () => {
          ...
          expect(newOutcomeShares.outcomes[1]).to.equal(0.02);
          expect(newOutcomeShares.outcomes[2]).to.below(
            outcomeShares.outcomes[2]
          );
          expect(newOutcomeShares.outcomes[2]).to.equal(0.02);
          expect(
          ...
          expect(myNewShares.outcomeShares[2]).to.above(
            myShares.outcomeShares[2]
          );
      ...
      it(
        ...
        mochaAsync(async () => {
          ...
          expect(newOutcomePrices.outcomes[2]).to.above(
            outcomePrices.outcomes[2]
          );
      ...
      for (const [odds, expectedDistribution] of initialOdds) {
        context(`Odds: ${odds}`, async () => {
          before(
            mochaAsync(async () => {
              const distribution =
                await predictionMarketContract.calcDistribution({ odds });
          ...
          it(
            "market prices should match odds",
            mochaAsync(async () => {
              const outcomePrices =
                await predictionMarketContract.getMarketPrices({ marketId });
          ...
          it(
            "market shares should match shares",
            mochaAsync(async () => {
              const outcomeShares =
                await predictionMarketContract.getMarketShares({ marketId });
          ...
          it(
            "market liquidity should match liquidity",
            mochaAsync(async () => {
              const marketData = await predictionMarketContract.getMarketData({
                marketId,
              });
